 ...
 */
async function getMetafieldMappingColumns() {
    try {
        // Get the selected product from resourceLocator
        const productResource = this.getCurrentNodeParameter('productId');
        if (!(productResource === null || productResource === void 0 ? void 0 : productResource.value)) {
            return [{
        ...
        const options = metafields.map((edge) => {
        ...
        return [{
                ...
                value: '',
                description: `Failed to load metafields: ${(error === null || error === void 0 ? void 0 : error.message) || 'Unknown error'}`,
            }];
    }
}
exports.getMetafieldMappingColumns = getMetafieldMappingColumns;
/**
 * Get appropriate icon for metafield type
 * Provides visual indicators for different metafield types
 */
function getMetafieldTypeIcon(type) {
    const typeMap = {
